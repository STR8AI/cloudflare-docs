...
## Bidirectional streaming with relatedRequestId
...
- **Responses and errors**: Automatically routed through the same stream as the request they are responding to, using the `message.id` field
- **Server-to-client requests**: Can be routed through a specific stream by providing a `relatedRequestId` option that references the original client request ID
- **Standalone notifications**: Sent through the standalone GET stream (if available) when no `relatedRequestId` is provided

### Using relatedRequestId for elicitation
...
export class MyMCP extends McpAgent {
  ...
  async init() {
    this.server.tool(
      ...
      {},
      async () => {
        // The SDK automatically handles relatedRequestId routing
        ...
        return {
          content: [{ type: "text", text: "Name not provided" }]
        };
      }
    );
  }
...
In this example, when the tool calls `elicitInput()`, the elicitation request is automatically sent through the same POST stream as the original tool call request, ensuring proper bidirectional communication. The client response to the elicitation is then routed back through the same stream.

### Advanced: Custom transport implementation
...
// Send a server-to-client request through the same stream as request "req-1"
...
The transport layer will prioritize routing based on:

1. **Response routing**: If the message has an `id` field matching a previous request (response or error), it uses that request's stream
2. **Related request routing**: If `relatedRequestId` is provided, the message is sent through the stream associated with that request
3. **Fallback**: If neither applies, the message is sent through the standalone GET stream
